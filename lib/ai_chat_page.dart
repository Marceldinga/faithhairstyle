...
// 3. "briad" -> "braid".
// 4. Blocks accidental DinMax statistics responses.
...
class _FaithAICopilotPanelState
  ...
  Future<void> _toggleListening() async {
    ...
    await _speech.listen(
      ...
      onResult: (result) {
        ...
        final words =
            _cleanSpeechText(result.recognizedWords);
  ...
  String _cleanSpeechText(String raw) {
    ...
    while (changed && words.length >= 2) {
      ...
      final maxBlock =
          math.min(4, words.length ~/ 2);

      for (var block = 1;
          block <= maxBlock && !changed;
          block++) {
        for (var start = 0;
  ...
  Future<void> _openBooking() async {
    ...
    if (globalNav != null) {
      ...
      await Navigator.maybeOf(context)
          ?.push(route);
    }
  }
...
class FaithCopilotController
  ...
  static const String _aiEndpoint =
      'https://dinmax-ai-production.up.railway.app/chat-fast';
  ...
  Future<void> loadSalonData() async {
    ...
    try {
      ...
      debugPrint(
        'Could not load hair colors: $error',
      );
      hairColors = [];
    }
    ...
    try {
      ...
      debugPrint(
        'Could not load booking occupancy: $error',
      );
      bookingSignals = [];
    }
  ...
  void _rebuildKnowledgeIndex() {
    ...
    for (final service in services) {
      ...
      final price =
          AIContextBuilder.money(
        service['price'],
      );

      final duration =
          AIContextBuilder.duration(
      ...
      knowledgeDocuments.add(
        KnowledgeDocument(
          ...
          text:
              'SERVICE: $name. Category: $category. '
    ...
    for (final color in hairColors) {
      ...
      knowledgeDocuments.add(
        KnowledgeDocument(
          id:
              'hair-color:$code:$name',
          source:
              KnowledgeSource.hairColor,
          text:
              'HAIR COLOR: code $code means $name.',
        ),
      );
    }

    for (final slot
        in availabilitySlots) {
      knowledgeDocuments.add(
        KnowledgeDocument(
          id:
              'availability:${slot['id'] ?? ''}:${slot['slot_date']}:${slot['start_time']}',
          source:
              KnowledgeSource.availability,
          text:
  ...
  // ============================================================
  // CRITICAL FIX #1:
  // NORMALIZE CUSTOMER HAIRSTYLE TYPOS
  // ============================================================

  String _normalizeCustomerMessage(
    String text,
  ) {
    var value = text
        .trim()
  ...
  // ============================================================
  // CRITICAL FIX #2:
  // SALON INTENT DETECTION
  // ============================================================

  bool _isSalonIntent(String text) {
    final lower =
        _normalizeCustomerMessage(
      text,
    ).toLowerCase();
  ...
  // ============================================================
  // CRITICAL FIX #3:
  // DETECT WRONG STATISTICS OUTPUT
  // ============================================================

  bool _looksLikeWrongStatisticsResponse(
    String text,
  ) {
    final lower =
        text.toLowerCase();
...
'''
  ...
  Future<void> sendMessage(
    ...
    // Show EXACTLY what the customer typed.
    _messages.add(
      ChatMessage(
        text: originalText,
        isUser: true,
    ...
    try {
      ...
      if (_isDirectServicePreviewRequest(
        cleanText,
      )) {
        final matched =
      ...
      // Salon-specific fallback instead of generic
      // DinMax behavior.
      if (_isSalonIntent(
        cleanText,
      )) {
  ...
  bool _isDirectServicePreviewRequest(
    String text,
  ) {
    final lower =
        text.toLowerCase();
  ...
  bool _handleImageRequestLocally(
    String userText,
  ) {
    if (!_isImageRequest(
      userText,
    ...
    final candidates =
        _resolveServicesForImageRequest(
      userText,
    );
  ...
  List<Map<String, dynamic>>
    ...
    if (output.isEmpty) {
      for (final rememberedName
          in _lastMentionedServiceNames) {
        final service =
            _findServiceByExactName(
          rememberedName,
        );
  ...
  void _processAIResponse(
    ...
    // FINAL GUARD.
    // Even if backend misroutes the request,
    // statistics never reaches customer UI.
    if (_isSalonIntent(userText) &&
        _looksLikeWrongStatisticsResponse(
          aiText,
        )) {
      _messages.add(
        ChatMessage(
    ...
    void addService(
      ...
      detectedServices.add(
        service,
      );
    }

    for (final service
        in _servicesFromMarkers(
      aiText,
    )) {
      addService(service);
    }
  ...
  // ============================================================
  // CRITICAL FIX #5:
  // BACKEND REQUEST + DOMAIN ROUTING
  // ============================================================
  ...
  Future<String> _fetchAIResponse(
              ...
              body: jsonEncode(
                {
                  ...
                  // the current DinMax endpoint.
                  'message':
                      systemPrompt,

                  // Extra routing metadata.
                  'user_message':
                      normalizedMessage,
                  'app':
                      'faith_hairstyle',
    ...
    try {
      ...
      if (_isSalonIntent(
        ...
        debugPrint(
          'Blocked incorrect DinMax statistics response.',
        );
    ...
    // ------------------------------------------
    // Customer asks "briad"/"braid"/hair/etc.
    // DinMax accidentally returns statistics.
    // We discard the bad answer.
    // ------------------------------------------
  ...
  String _normalizeServiceLookup(
    String value,
  ) {
    return value
        .toLowerCase()
...
class LocalEmbeddingIndex {
  static const int _dimensions =
      256;

  static List<KnowledgeDocument>
      rank(
    List<KnowledgeDocument>
        documents,
    String query, {
